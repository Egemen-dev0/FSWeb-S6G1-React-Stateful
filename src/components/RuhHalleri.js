...
export default function RuhHalleri() {
  /* ADIM 1 */
	let [ruhhalleri, setruhhalleri] = useState(ilkRuhHali);
	
  const mutluEt = () => {
    setruhhalleri(mutluRuhHali)
  };
  const uZ = () => {
    /* ADIM 5 */
    setruhhalleri(uzgunRuhHali)
  };
  const reset = () => {
    /* ADIM 6 */
    setruhhalleri(ilkRuhHali)
  };

  const stil = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color:  ruhhalleri == mutluRuhHali ? 'royalblue' : "crimson", /* ADIM 2 */
  };

  return (
    <div className='widget-moods container'>
      <h2>RuhHalleri</h2>
	<div id='ruhHali' style={stil}>{ruhhalleri}</div>
      <div>
